...
//TODO, try using uniform buffer instead of int	d_gcrm_faceCorners[24]
/*
uniform FaceToCornerId{
int data[14];
}faceToCornerId;
*/

/*
__constant__ int d_gcrm_faceCorners[24]={
...
bool ReloadVtxInfo(in int triangle_id, in int ilayer, inout GCRMPrism prism)
{
	...
	//bool normalCase = ((edge1S == iS) && (edge1E != iE)) || ((edge1S == iE) && (edge1E != iS));
	...
	inout HitRec tOutRec, inout int nextCellId)
{
	...
	{//Special case when ray hit on the edge.
		nextCellId = -1;//getAdjacentCellIdGCRM(m_meshCellId, tOutRec.hitFaceid,prevCellId);//
	}
...
void ComputeVerticesNormal(inout GCRMPrism prism, inout RealType3 vtxNormals[3])
{
	...
	for (int iCorner = 0; iCorner < 3; iCorner++){
		...
		for (int iPrism = 0; iPrism < nNeighbors; iPrism++){
...
#define  MASK_RGBA_A(x)  ((0xff000000&(x))>>24)
#define  MASK_RGBA_B(x)  ((0x00ff0000&(x))>>16)
#define  MASK_RGBA_G(x)  ((0x0000ff00&(x))>>8)
#define  MASK_RGBA_R(x)  ((0x000000ff&(x)))
...
void main(){
	...
	/*
	uint R = MASK_RGBA_R(uint(triangle_id));
	uint G = MASK_RGBA_G(uint(triangle_id));
	uint B = MASK_RGBA_B(uint(triangle_id));
	uint A = MASK_RGBA_A(uint(triangle_id));
	...
	*/
	//out_Color = vec4(vec3(tInHitRecord.t)/35.0, 1.0);
	//return;
